...
const getJson = url => fetch(url).then(response => response.json()).then(resultados => {  
  let cohortListId = resultados.map(element => {
    return element;
  })
    console.log(cohortListId);
    printCohorts(cohortListId);
